...
private:
    ...
    bool FulfillsITSHitRequirements(uint8_t itsClusterMap) {
        ...
        for (auto& itsRequirement : mRequiredITSHits) {
            ...
            if ((itsRequirement.first == -1) && (hits > 0)) { return false; }  // no hits were required in specified layers
            if (hits < itsRequirement.first) { return false; }  // not enough hits found in specified layers
            }
        return true;
        }

    const std::string mCutNames[static_cast<int>(TrackSelection::TrackCuts::kNCuts)] = {"TrackType", "PtRange", "EtaRange", "TPCNCls", "TPCCrossedRows", "TPCCrossedRowsOverNCls", "TPCChi2NDF", "TPCRefit", "ITSNCls", "ITSChi2NDF", "ITSRefit", "ITSHits", "GoldenChi2", "DCAxy", "DCAz"};
