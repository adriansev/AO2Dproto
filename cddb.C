...
class CcdbApi //: public DatabaseInterface
{
 public:
  ...
  virtual ~CcdbApi() { curl_global_cleanup(); }

  void curlInit() {
    // todo : are there other things to initialize globally for curl ?
    curl_global_init(CURL_GLOBAL_DEFAULT);
    // mJAlienCredentials.reset(new TJAlienCredentials());
    mJAlienCredentials->loadCredentials();
    mJAlienCredentials->selectPreferedCredentials();
    }
